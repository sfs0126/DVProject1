...
plot3 <- ggplot(data = df.students, aes(x=`reading score`, y=100*(..count../sum(..count..)))) +
  geom_histogram(color="black", fill="paleturquoise", binwidth = 5)  +
...
plot4 <- ggplot(data=df.students, aes(x=`math score`, y = 100*(..count../sum(..count..)), fill=`test preparation course`)) +
  geom_histogram(color = 'black', binwidth = 5) + labs(title="Math Scores by Test Preparation", subtitle = "Based on 1,000 US High School Students", x="Score", y="Percent (%)", caption = "Data From Kaggle: Student Performance")  + scale_y_continuous(limit = c(0, 10), expand=c(0,0)) + 
